...
import {
  ...
  LogOut, PanelLeftClose, Pill, Search,
} from "lucide-react";
...
export default function Sidebar() {
  ...
  return (
    ...
    >
      <div className={cn(
        "flex items-center h-13 relative",
        collapsed ? "justify-center" : "px-3.5 gap-2.5"
      )}>
        <div className="flex items-center justify-center rounded-lg h-7.5 w-7.5 bg-sidebar-primary/10 shrink-0">
        ...
        <AnimatePresence>
          {!collapsed && (
            ...
            >
              <p className="text-sm font-semibold text-sidebar-foreground truncate tracking-tight">Faraz Pharmacy</p>
              <p className="text-[9px] text-sidebar-foreground/40 truncate tracking-widest uppercase">Management</p>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
      ...
      <div className={cn("px-2 pb-2", collapsed && "px-1")}>
        <button
          ...
          className={cn(
            ...
            "bg-sidebar-primary text-white hover:bg-sidebar-primary/90 shadow-sm",
            collapsed ? "justify-center h-8" : "gap-2 px-2.5 h-8"
          )}
        >
            ...
            >
              <button
                ...
                className={cn(
                  ...
                  collapsed ? "justify-center h-8" : "gap-2.5 px-2.5 h-8",
                  isActive
      ...
      <div className="border-t border-sidebar-border px-2 pt-2 pb-3 space-y-1">
        <div className={cn(
          "flex items-center rounded-md px-2.5 py-1.5 hover:bg-sidebar-accent/50 transition-colors cursor-pointer",
          ...
          <AnimatePresence>
            {!collapsed && (
              ...
              >
                ...
                <p className="text-[9px] text-sidebar-foreground/40 truncate tracking-wider uppercase leading-tight">Admin</p>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
      ...
      <button
        ...
        className={cn(
          "absolute -right-3 top-13 h-5 w-5 rounded-full border border-border bg-surface flex items-center justify-center text-text-secondary hover:text-text-primary transition-all duration-200 z-20 shadow-xs",
