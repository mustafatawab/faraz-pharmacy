...
export default function Returns() {
  ...
  const { data: sales = [] } = useQuery({ queryKey: ["recent-sales"], queryFn: () => api.sales.listRecent(20) });
  ...
  return (
    <div>
      ...
      <Dialog open={open} onOpenChange={(v) => { if (!v) { setSelectedSaleId(""); setReason(""); setReturnQtys({}); } setOpen(v); }}>
        <DialogContent className="max-w-lg">
          <DialogHeader><DialogTitle>New Return</DialogTitle></DialogHeader>
          <div className="space-y-4">
            <div>
              <Label>Sale</Label>
              <Select value={selectedSaleId} onValueChange={(v) => { setSelectedSaleId(v); setReturnQtys({}); }}>
                <SelectTrigger><SelectValue placeholder="Select sale" /></SelectTrigger>
                <SelectContent>
                  {sales.map((s: Sale) => (
                    <SelectItem key={s.id} value={s.id}>{s.id.slice(0, 8)} — {formatCurrency(s.total)} ({s.customer_name || "Walk-in"})</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
                    ...
                    return (
                      <div key={item.product_id} className="flex items-center gap-2">
                        ...
                        <div className="flex items-center gap-1">
                          <button onClick={() => setReturnQtys(prev => ({ ...prev, [item.product_id]: Math.max(0, (prev[item.product_id] || 0) - 1) }))} className="h-7 w-7 rounded-md bg-surface-2 flex items-center justify-center hover:bg-border"><Minus className="h-3 w-3" /></button>
                          <span className="w-6 text-center text-sm font-mono">{qty}</span>
                          <button onClick={() => setReturnQtys(prev => ({ ...prev, [item.product_id]: Math.min(item.quantity, (prev[item.product_id] || 0) + 1) }))} className="h-7 w-7 rounded-md bg-surface-2 flex items-center justify-center hover:bg-border"><Plus className="h-3 w-3" /></button>
                        </div>
                      </div>
                    );
                  })}
            ...
            <Button className="w-full" disabled={!selectedSaleId || !reason || Object.values(returnQtys).every(q => q === 0)} onClick={() => createMutation.mutate()}>
